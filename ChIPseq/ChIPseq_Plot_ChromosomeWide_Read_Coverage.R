...
chromosome_density=function(x,y,z){
  ...
  #####Order for plotting: first plot TAM as it is higher and should be at the back
  #####The order for plotting can be switched using factor levels
  ...
  p=ggplot(data = plot, aes(y=value, x=coordinate, fill=variable))+
    ...
    scale_fill_manual(values=c('#ed2224', '#29286b'), labels=c('TAM', 'UNT'))+scale_alpha_manual(values = c(1, 1), guide='none')+
    ggtitle(paste0(z, ' in ', x))+theme(plot.title = element_text(size=15, hjust = 0.5))+
